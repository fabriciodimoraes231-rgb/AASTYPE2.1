...
class SimpleIoTSimulator:
    ...
    def generate_sensor_data(self):
        """Gera dados dos 4 sensores conforme model.json"""
        timestamp = datetime.now().isoformat()
        
        # 1. TEMPERATURE (15°C a 35°C) - igual ao model.json
        ...
        # 2. HUMIDITY (30% a 85%) - igual ao model.json
        ...
        # 3. NOISELEVEL (20dB a 90dB) - igual ao model.json
        noiselevel = round(random.uniform(20.0, 90.0), 1)
        noiselevel_data = {
            "value": noiselevel,
            "unit": "dB",
            "timestamp": timestamp
        ...
        # 4. STATUS - igual ao model.json (string simples)
        ...
        return temp_data, humidity_data, noiselevel_data, status_data

    def publish_data(self):
        """Publica os dados no MQTT"""
        try:
            # Gera dados dos sensores
            temp_data, humidity_data, noiselevel_data, status_data = self.generate_sensor_data()
            
            # Publica cada variável em seu tópico (nomes iguais ao model.json)
            ...
            self.client.publish(f"{self.topic_prefix}/noiselevel", json.dumps(noiselevel_data))
            self.client.publish(f"{self.topic_prefix}/status", json.dumps(status_data))
            
            # Log simples
            print(f"📊 Temperature: {temp_data['value']}°C | "
                  f"Humidity: {humidity_data['value']}% | "
                  f"NoiseLevel: {noiselevel_data['value']}dB | "
                  f"Status: {status_data['value']}")
    ...
    def start(self):
        ...
        print(f"📝 Tópicos: {self.topic_prefix}/[temperature|humidity|noiselevel|status]")
